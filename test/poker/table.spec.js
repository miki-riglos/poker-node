...
describe('Table class', function() {
  ...
  describe('Registration', function() {
    ...
    it('should allow players registration', function() {
      table.registerPlayer(1, 'Sofia').errorMessage.should.be.empty;
      table.registerPlayer(2, 'Bianca').errorMessage.should.be.empty;
      keys(table.players).should.have.lengthOf(2);
    });

    it('should validate position uniqueness', function() {
      table.registerPlayer(1, 'Sofia').errorMessage.should.be.empty;
      table.registerPlayer(1, 'Bianca').errorMessage.should.equal('Position 1 already taken');
    });

    it('should validate position minimum and maximum', function() {
      table.registerPlayer(0, 'Sofia').errorMessage.should.not.be.empty;
      table.registerPlayer(11, 'Sofia').errorMessage.should.not.be.empty;
    });
    ...
    it('should allow registration only if table has not started', function() {
        ...
        table.registerPlayer(2, 'Bianca').errorMessage.should.be.empty;
        table.start();
        table.registerPlayer(3, 'Giovana').errorMessage.should.not.be.empty;
    });
    ...
    it('should not allow registration if maximum number of players are registered', function() {
      ...
      table.registerPlayer(11, 'Player 11').errorMessage.should.not.be.empty;
    });
